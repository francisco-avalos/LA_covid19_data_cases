...
# Wed March 24, 2021
def return_cases_ResCong(pat1, pat2, pat3, pat4, pat5):
    """
    Function scans the LA Public Health Care website and returns a list of the targetted data information. It's
    particularly designed to capture the Residential Congregate data since the title web-page is designed with several indentions
...
# Wed March 24, 2021
def return_cases_educational(pat1, pat2, pat3):
    """
    Function scans the LA Public Health Care website and returns a list of the targetted data information. It's
    particularly designed to capture the homeless data since the title web-page is designed with several indentions
    Input: string variables
    ...
    while (i <= maxi):
        if((soup_strings[i] == str(pat1)) & (soup_strings[i+1] == str(pat2)) & (soup_strings[i+2] == str(pat3))):
            length = len(soup_strings[i:])
            start = len(soup_strings) - length
            end = start + length
            stop = 'Total'
